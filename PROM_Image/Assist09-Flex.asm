...
RAMOFS  EQU     -$7000          ; ROM OFFSET TO RAM WORK PAGE
ROMSIZ  EQU     $2000           ; ROM SIZE
ROMS2IZ EQU     $2000            ; ROM SIZE
ROM2OF  EQU     ROMBEG-ROM2SIZ  ; START OF EXTENSION ROM
ACIA    EQU     $F230           ; DEFAULT ACIA ADDRESS
...
*****************************************************
* FIRST STEP IS TO MOVE THE CONTENTS OF THE ROM INTO RAM, AND THEN 
* DISABLE THE ROM. DONE IN TWO STEPS TO AVOID THE IO SPACE
*****************************************************
        LDA		#$0E			; SET THE 4K PAGE REGISTER
        STA		$F280
		LDY		#$8000			; BASE ADDRESS OF SOURCE/DESTINATION AREA 
		LDX		#$7000			; NUMBER OF BYTES TO MOVE
COPY1	LDA		,Y				; GET NEXT BYTE FROM SOURCE
		STA		,Y+				; MOVE IT TO DESTINATION
		LEAX	-1,X			; DECREMENT BYTE COUNTER
		BNE		COPY1			; CONTINUE UNTIL COUNTER = 0
		
		LDY		#$F800			; BASE ADDRESS OF SOURCE/DESTINATION AREA 
		LDX		#$800			; NUMBER OF BYTES TO MOVE
COPY2	LDA		,Y				; GET NEXT BYTE FROM SOURCE
		STA		,Y+				; MOVE IT TO DESTINATION
		LEAX	-1,X			; DECREMENT BYTE COUNTER
		BNE		COPY2			; CONTINUE UNTIL COUNTER = 0

        LDA		#$80			; SWITCH-OFF THE ROM
        STA		$F34E
*****************************************************
...
SIGNON  FCC     /ASSIST09-Flex/ ; SIGNON EYE-CATCHER
        FCB     EOT
...
CIDTA   LDU     #PORTA			; LOAD ACIA ADDRESS
...
CODTA   PSHS    U,D,CC          ; SAVE REGISTERS,WORK BYTE
        LDU     <VECTAB+.ACIA   ; ADDRESS MC68681
...
CODTAO	LDB     $F341              ; LOAD ACIA CONTROL REGISTER
		BITB    #$04            ; ? TX REGISTER CLEAR >LSAB FIXME
...
SERINIT LDX		#PORTA
        ...
        STA		ModeReg, X
        LDA		#%01110000		; Set 1, xternal x16 CLK
        STA		AuxCont, X
